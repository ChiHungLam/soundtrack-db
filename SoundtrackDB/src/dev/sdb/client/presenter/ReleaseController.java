...
public class ReleaseController extends AbstractDataController implements ReleaseSearchView.Presenter,
	...
	@Override protected DetailView createDetailWidget() {
		...
		initReleaseSoundtrackListTable(view);
		return view;
	}

	private void initReleaseSoundtrackListTable(final ReleaseDetailView view) {
		//		final ReleaseDetailWidget widget = new ReleaseDetailWidget(this);
		DataGrid<Entity> table = view.getSublist().getTable();
	...
	public void getSequenceListFromServer(final ReleaseDetailView detailWidget) {
		final SublistWidget list = detailWidget.getSublist();

		Release release = (Release) getCurrentEntity();
		if (release == null) {
	...
	@Override public void searchRelease(String term) {
